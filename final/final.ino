...
// This project was tested with AI Thinker ESP32-CAM
...
void startCameraServer(){
  ...
  Serial.printf("Snapshot: http://%s:%d/capture\n", WiFi.localIP().toString().c_str(), config.server_port);
}
...
void setup() {
  ...
  if(psramFound()){
    ...
    config.frame_size = FRAMESIZE_VGA;   // safer than SVGA/UXGA
    config.jpeg_quality = 12;
    config.fb_count = 1;                 // keep single buffer
  } else {
