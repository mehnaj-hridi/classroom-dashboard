...
rfidUsers.forEach((user) => {
  db.run("INSERT OR IGNORE INTO rfid_users (uid, name) VALUES (?, ?)", [user.uid, user.name], (err) => {
    if (err) {
  ...
  try {
    ...
    parser.on("data", (data) => {
      ...
      if (
        ...
        db.run("INSERT INTO motion_logs (time) VALUES (?)", [time], (err) => {
          if (err) {
...
app.get("/attendance", (req, res) => {
  ...
  db.all("SELECT * FROM attendance_logs WHERE date = ? ORDER BY id DESC LIMIT 50", [today], (err, rows) => {
    if (err) {
...
app.get("/motion", (req, res) => {
  const today = new Date().toLocaleDateString()
  db.all("SELECT * FROM motion_logs WHERE date(timestamp) = date('now') ORDER BY id DESC LIMIT 50", (err, rows) => {
    if (err) {
...
app.get("/last-motion", (req, res) => {
  db.get("SELECT * FROM motion_logs ORDER BY id DESC LIMIT 1", (err, row) => {
    if (err) {
...
app.get("/stats", (req, res) => {
  ...
  db.get(
    ...
    (err, attendanceRow) => {
      ...
      db.get("SELECT COUNT(*) as count FROM motion_logs WHERE date(timestamp) = date('now')", (err, motionRow) => {
        if (err) {
        ...
        db.get("SELECT time FROM motion_logs ORDER BY id DESC LIMIT 1", (err, lastMotionRow) => {
          if (err) {
...
app.post("/users", (req, res) => {
  ...
  db.run("INSERT OR REPLACE INTO rfid_users (uid, name) VALUES (?, ?)", [uid.toUpperCase(), name], function (err) {
    if (err) {
